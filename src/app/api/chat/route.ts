...
export async function POST(req: Request) {
  try {
    ...
    // Use storytelling prompt if no custom instructions provided (Story mode)
    // Check if it's a default message (just location name or "Research the history of X")
    const isDefaultMessage = location && !researchQuery.includes('\n\n') && (
      researchQuery === `Research the history of ${location.name}` ||
      researchQuery === location.name ||
      researchQuery.startsWith('Research the history of')
    );

    console.log('[Chat API] isDefaultMessage:', isDefaultMessage);
    console.log('[Chat API] location:', location);
    console.log('[Chat API] researchQuery:', researchQuery);

    if (isDefaultMessage) {
      // Import storytelling prompt generator
      const { generateStorytellingPrompt } = await import('@/lib/storytelling-prompt');
      
      researchQuery = generateStorytellingPrompt({
        locationName: location.name,
        lat: location.lat,
        lng: location.lng,
      });
      
      console.log('[Chat API] ✓ Using storytelling prompt, length:', researchQuery.length);
      console.log('[Chat API] Prompt preview:', researchQuery.substring(0, 200));
    } else {
      console.log('[Chat API] ✗ NOT using storytelling prompt - custom instructions detected');
    }
